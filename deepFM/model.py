...
class Model(object):
    def __init__(self, user_count, item_count, cate_count, cate_list):
        ...
        hist = tf.div(
            hist, tf.cast(tf.tile(tf.expand_dims(self.sl, 1), [1, 128]), tf.float32)
        )
        ...
        d_layer_fm_j = tf.layers.dense(
            d_layer_fm_j, 1, activation=None, name="f_fm", reuse=True
        )
        ...
        # logits for all item:
        all_emb = tf.concat(
            [item_emb_w, tf.nn.embedding_lookup(cate_emb_w, cate_list)], axis=1
        )
        ...
        d_layer_fm_all = tf.layers.dense(
            d_layer_fm_all, 1, activation=None, name="f_fm", reuse=True
        )
    ...
    def eval(self, sess, uij):
        u_auc, socre_p_and_n = sess.run(
            [self.mf_auc, self.p_and_n],
            feed_dict={
                self.u: uij[0],
                self.i: uij[1],
                self.j: uij[2],
